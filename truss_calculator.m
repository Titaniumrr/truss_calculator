...
refreshAll();
    ...
    function buildPlotPanel()
        ...
        xlabel(ui.axes, 'X');
        ylabel(ui.axes, 'Y');
        title(ui.axes, 'Unverformtes Fachwerk');
    end
    ...
    function refreshPlot()
        ...
        if isReady
            titleText = 'Unverformtes Fachwerk';
        end
        if ~isempty(state.results) && isReady
            titleText = sprintf('Verformtes Fachwerk (Verformung x %.3g)', state.results.scale);
        end
        ...
        try
            ...
            if usejava('desktop')
                progressHandle.waitbar = waitbar(0.05, 'Computation starts...', ...
                    'Name', 'Fachwerkrechner', 'CreateCancelBtn', @onComputeCancelRequested);
            end
        catch
        end
    end
